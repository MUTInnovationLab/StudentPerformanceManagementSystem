...
export class StrugglingStudentsPage implements OnInit {
  ...
  async ngOnInit() {
    this.auth.onAuthStateChanged((user) => {
      if (user && user.email) {
        this.getAssignedModules(user.email);
  ...
  async getAssignedModules(userEmail: string) {
    try {
      // Query all assignedLectures documents
      ...
      // Process each document to find modules for this lecturer
      assignedLecturesSnapshot.docs.forEach(doc => {
        const data = doc.data() as AssignedLectureData;
        if (data && Array.isArray(data.modules)) {
          // Filter modules assigned to this user's email
          const userModules = data.modules.filter(
      ...
      this.assignedModules = allModules;
      ...
      if (this.assignedModules.length === 0) {
        ...
        this.presentToast('No assigned modules found', 'warning');
      }
    } catch (error) {
